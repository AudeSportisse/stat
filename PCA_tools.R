...
# To scale only the data variables and not the missing data pattern variables. 
imputePCAmodif <- function (X,nbcol, ncp = 2, scale = TRUE, method = c("Regularized", 
                                                                       "EM"), row.w = NULL, coeff.ridge = 1, threshold = 1e-06, 
                            seed = NULL, nb.init = 1, maxiter = 1000, ...) 
{
  impute <- function(X, nbcol, ncp = 4, scale = TRUE, method = NULL, 
    ...
    Xhat <- t(t(X) - mean.p)
    et <- apply(Xhat[,1:nbcol], 2, ec, row.w)
    if (scale==TRUE) 
      Xhat[,1:nbcol] <- t(t(Xhat[,1:nbcol])/et)
    if (any(is.na(X))) 
      Xhat[missing] <- 0
    if (init > 1) 
    ...
    while (nb.iter > 0) {
      ...
      sigma2 <- nrow(X) * ncol(X)/min(ncol(X), nrow(X) - 
                                        1) * sum((svd.res$vs[-c(1:ncp)]^2)/((nrow(X) - 
                                                                               1) * ncol(X) - (nrow(X) - 1) * ncp - ncol(X) * 
                                                                              ncp + ncp^2))
    ...
    if (scale) 
      fittedX[,1:nbcol] <- t(t(fittedX[,1:nbcol]) * et)
  ...
  for (i in 1:nb.init) {
    if (!any(is.na(X))) 
      return(X)
    res.impute = impute(X,nbcol=nbcol, ncp = ncp, scale = scale, method = method, 
